...
- **주관으로 판단하지 않는다** — 단가·비용 delta를 기억으로 추정하지 않는다. 조회할 수 없으면 "조회 불가"로 남기고 추정치를 확정 수치처럼 제시하지 않는다.
- **MCP를 먼저 활용한다** — AWS Billing MCP(실제 청구·이상 감지) / `aws-pricing`(단가) → AWS 공식 문서 → AWS 공식 블로그 → 프로젝트 `docs/` 순으로 조회한 근거 위에서 분석한다.
- **확인분과 추론분을 구분한다** — 실측 청구 데이터, 공식 단가표, 사용량 가정을 각각 구분해 표기한다. 특히 사용량 가정(트래픽·실행 시간 등)은 근거를 함께 밝힌다.
