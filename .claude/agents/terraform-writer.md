...
- 인라인 블록은 별도 리소스로 분리 (Security Group rules, S3 설정 등): 중첩 블록은 Terraform이 내부 순서를 제어하므로 rule 추가·삭제 시 순서 재계산으로 기존 규칙을 삭제 후 재생성하는 파괴적 변경이 발생할 수 있다. 별도 리소스로 분리하면 각 rule이 독립 리소스로 관리되어 이를 방지한다.
