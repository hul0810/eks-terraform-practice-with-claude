...
- **주관으로 판단하지 않는다** — "보통 이렇게 한다" 수준의 근거로 리소스 인자·버전·기본값을 정하지 않는다. 근거를 찾을 수 없으면 모른다고 말하고 확인 항목으로 남긴다.
- **MCP를 먼저 활용한다** — Terraform Registry(`terraform` MCP) → AWS 공식 문서(`aws-knowledge-mcp-server`) → AWS 공식 블로그 → 프로젝트 `docs/` → 업스트림 소스(`.terraform/` 하위 실제 모듈 코드) 순으로 조회한 근거 위에서 코드를 작성한다. 존재 여부가 불확실한 인자는 반드시 스키마로 확인한다.
- **확인분과 추론분을 구분한다** — 주석·설명에 쓰는 수치·조건에는 출처를 밝히고, 문서로 확인하지 않은 항목은 "미확인"으로 표기한다.

> 상세: `docs/terraform-principles.md` → "근거 기반 작업 원칙"

## 엔지니어링 철학

`docs/terraform-principles.md`의 "엔지니어링 철학" 절이 단일 진실 공급원이다 — 코드 작성 전
반드시 그 절을 먼저 읽는다. 핵심 절차만 요약하면: **단순함이 출발점**(최소 변경으로 목표
달성 가능한지 먼저 확인) → 그 접근이 확장성/가독성/효율성/장기 유지비용 중 하나라도
해치는지 점검 → 해친다면 처음부터 올바른 구조로(설정 비용 낮고 장기 효과 높을 때) →
기존 코드 개선도 장기 이득이면 지금 한다. "오버엔지니어링"과 "과소설계" 둘 다 사용자의
설계 의도와 맥락을 충분히 파악한 후에만 판단한다.
