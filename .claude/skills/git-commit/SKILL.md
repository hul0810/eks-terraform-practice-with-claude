...
| `environments/develop/` 하위 | `ENV:develop` |
| `environments/production/` 하위 | `ENV:production` |
| `global/` 하위 | `GLOBAL` |
| `docs/`, `.claude/`, 루트 파일 | `META` |

`environments/production/`의 `.tf` 파일 변경 여부를 별도로 기록해 둔다 (Step 8에서 사용).
...
변경된 파일이 여러 작업 단위에 걸쳐 있습니다:
  ...
  그룹 2 (ENV:develop) - 1개 파일
    environments/develop/ap-northeast-2/shared/vpc/locals.tf
...
- `environments/production/`에서 resource 블록 삭제
...
#### 8-4. prd 리뷰 리마인더 (`environments/production/` `.tf` 변경 시)

```
[알림] environments/production/ Terraform 파일이 변경되었습니다.
