locals {
  ...
  eks = {
    ...
    # replicaCount는 기본값(2)을 유지하고 배치 강제만 준다.
    # 단 시스템 노드가 desired=1인 동안에는 podAntiAffinity가 preferred라 두 replica가 같은 노드에
    # 뜬다 — replica 2가 노드 장애를 막아주지는 못하고, CA가 노드를 늘린 뒤 재스케줄되는 시점부터
    # 분산된다. AZ 단위 분산까지 필요하면 topologySpreadConstraints를 함께 지정한다.
    #
    ...
    # cert-manager: CriticalAddonsOnly toleration + role=system nodeAffinity로 시스템 노드 배치.
    # replica는 기본값(2) 유지. 3개 컴포넌트가 동일한 배치 규칙을 쓴다.
