...
#
# project/environments/develop/.../eks-addons/gitops-bridge-registry.tf를 그대로 본떴다 —
# Hub(monitoring)가 이 클러스터를 spoke로 등록하려면 엔드포인트/CA/spoke Role ARN/addon
# IAM 메타데이터를 알아야 하는데, 그 값을 이 클러스터가 직접 SSM Parameter Store(Standard
# tier, String)에 발행한다. Hub는 그 경로를 discovery만 한다
# (monitoring/.../eks-addons/gitops-bridge-registry.tf) — 이 root가 dev/prod 어느 쪽이든
# Hub 코드는 바뀌지 않는다.
#
