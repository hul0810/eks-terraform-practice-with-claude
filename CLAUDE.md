...
## 근거 기반 작업 원칙 (MCP 사용 규칙 포함)

**핵심 선언: 모든 판단은 검증 가능한 근거 위에서 내린다.**

1. **주관으로 판단하지 않는다.** "보통 이렇게 한다", "아마 이 값일 것이다" 수준의 근거로 설계 결정·버전·설정값을 정하지 않는다. 근거를 찾을 수 없으면 **모른다고 말하고 확인 항목으로 남긴다** — 추정치로 빈칸을 메우지 않는다.
2. **MCP가 설정되어 있으면 반드시 먼저 활용한다.** 공식 문서 및 공신력 있는 기술 자료(AWS 공식 블로그 등)를 근거로 검토하고 작업 방향을 잡는다. 개인 블로그·LLM 기억은 이를 대체하지 않는다.
3. **확인분과 추론분을 구분해 표기한다.** 문서로 확인한 사실과 훈련 데이터 기반 추론을 같은 톤으로 섞지 않는다. 미확인분은 "문서로 확인하지 않음"을 명시한다.
...
- 조회 우선순위·표기 규칙 상세: `docs/terraform-principles.md` → "근거 기반 작업 원칙" 참조
