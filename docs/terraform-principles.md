...
아래 네 절은 순서대로 적용하는 하나의 의사결정 절차다 — "단순함이 출발점"이 기본값이고,
나머지는 그 기본값을 언제 벗어나야 하는지를 규정한다.

### 1. 단순함이 출발점
목표에 도달할 수 있는 가장 단순한 방법을 먼저 찾는다. 수정이 필요할 때도 마찬가지다 —
가장 작고 최소한의 변경으로 목표에 도달할 수 있는지 먼저 확인한다. 불필요한 추상화,
당장 쓰이지 않는 파라미터, "나중에 필요할 수도 있는" 확장 포인트는 그 자체로 복잡도
비용이다 — 아래 2절의 기준을 실제로 해치지 않는 한 미리 만들어두지 않는다.

### 2. 그러나 아래 네 기준 중 하나라도 해치면 안 된다
단순함은 출발점이지 목표가 아니다. 1절에서 고른 단순한 접근이 아래 중 하나라도 명백히
해친다면 그 접근을 채택하지 않는다:
- **확장성** — 환경·리소스가 늘어날 때 기존 코드를 건드리지 않고 확장 가능한가
- **가독성** — 미래의 작업자(혹은 미래의 자신)가 코드만 보고 의도를 파악할 수 있는가
- **효율성** — 반복적인 수동 개입이나 갱신 작업을 요구하지 않는가
- **장기 유지비용** — 지금의 단순함이 나중에 더 큰 리팩토링·이중 구조 운영 부담으로
  돌아오지 않는가

  **예시** (2026-07-24, 이 프로젝트에서 실제로 겪은 사례): monitoring NAT Gateway의 공인
  IP를 develop EKS의 접근 허용 목록에 문자열로 하드코딩한 적이 있다. 코드 한 줄이라는
  점에서는 가장 "단순"해 보였지만, 이 IP는 monitoring을 재생성할 때마다 바뀌어 매번 수동
  갱신이 필요했고(효율성 훼손), 갱신을 깜빡하면 Hub→spoke 연결이 에러 없이 끊기는
  장애로 이어졌다(장기 유지비용 훼손) — 표면적 단순함이 실제로는 반복 비용을 감춘
  것이었다. `data "aws_nat_gateway"`로 동적 조회하도록 바꾼 뒤에는 이 문제 자체가 구조적으로
  사라졌다. 반대로 같은 종류의 값이라도 재생성해도 바뀌지 않는 결정론적 값(예: Hub IAM Role
  ARN)은 하드코딩이 네 기준 중 어느 것도 해치지 않으므로 그대로 두는 게 맞다 — "지금 안
  바뀌니 안전"이 아니라 "구조적으로 안 바뀐다는 게 증명 가능한가"가 기준이다.

### 3. 해친다면 — 처음부터 올바른 구조로
설정 비용이 낮고 장기 효과가 높다면, 현재 규모나 복잡도와 관계없이 올바른 구조를 처음부터
갖춘다. 구조 없이 시작하면 기능이 쌓일수록 대규모 리팩토링, 호환성 검증, 이중 구조 운영
부담이 생긴다. "지금은 단순하게"가 "나중에 복잡하게"를 만드는지 2절의 네 기준으로 항상
먼저 검토한다.

### 4. 리팩토링이 장기적으로 이득이면 미루지 않는다
기존 코드를 손보는 경우도 같은 절차를 따른다. 지금 당장 동작하는 수준을 유지하는 것과
장기적으로 효율적인 구조로 정리하는 것 중 후자가 이득이라면, 지금 하는 작업 범위가
아니라는 이유로 미루지 않고 이번 변경에 포함한다. "일단 동작하니 나중에"는 부채를
이자와 함께 미루는 것과 같다.

---

### 예외 없이 항상 기본값인 것들
아래는 위 1절("단순함이 출발점")의 적용 대상이 아니다 — 상황을 따지지 않고 항상 기본값으로
갖춘다. "지금은 필요 없으니 생략"이 통하지 않는 항목이라는 뜻이다.
- 모듈 버전 관리 (`docs/terraform-principles.md` "버전 관리" 절)
- State 격리 (환경별 backend 분리)
- 명시적 인터페이스 (`variable`/`output` 선언, `description`)
...
**모든 판단은 검증 가능한 근거 위에서 내린다. 주관·직관·훈련 데이터 기억으로 작업 방향을 정하지 않는다.**

### 1. 주관으로 판단하지 않는다

"보통 이렇게 한다", "아마 이 값일 것이다", "예전에 이랬다" 수준의 근거로 설계 결정·버전 선택·설정값을 정하지 않는다.
근거를 찾을 수 없으면 **모른다고 말하고 확인 항목으로 남긴다.** 추정치로 빈칸을 메우지 않는다.

### 2. MCP가 설정되어 있으면 반드시 먼저 활용한다

MCP는 선택지가 아니라 1차 조회 수단이다. 조회 우선순위:

| 순위 | 출처 | 용도 |
|------|------|------|
| 1 | AWS 공식 문서 (`aws-knowledge-mcp-server` MCP) | 서비스 동작·제약·설정 스키마·베스트 프랙티스 |
| 2 | Terraform Registry (`terraform` MCP) | provider/모듈 최신 버전, 리소스·변수 스키마 |
| 3 | AWS 공식 블로그 (AWS Containers Blog 등) | 문서에 없는 동작 배경·수치·설계 의도 |
| 4 | 프로젝트 문서 (`docs/`, 모듈 `CLAUDE.md`) | 이 프로젝트의 확정된 결정·제약 |
| 5 | 업스트림 소스 코드 (`.terraform/` 하위 모듈, 공개 Helm 차트 values) | 문서에 없는 실제 노출 필드·기본값 |

개인 블로그·Stack Overflow·LLM 기억은 위 출처를 대체하지 않는다. 참고했다면 그 사실을 밝히고 공식 출처로 교차 확인한다.

### 3. 확인분과 추론분을 구분해 표기한다

설명·리뷰·문서에 **검증된 사실과 추론을 같은 톤으로 섞지 않는다.**

- **확인분**: 수치·조건·필드명은 출처를 함께 밝힌다. 해석이 갈리는 대목은 원문을 인용한다.
- **미확인분**: "문서로 확인하지 않음"을 명시한다. 상위 Kubernetes 기본값, 소요 시간 추정치,
  AWS 외 프로젝트(Cluster Autoscaler 등) 동작은 AWS 문서 범위 밖이라 별도 확인이 필요하다.
- **negative inference 구분**: "문서에 방법이 없다"와 "문서가 금지한다"는 다르다. 전자를 후자처럼 단정하지 않는다.

### 4. 결론을 반영하기 전에 다시 갈라낸다

긴 논의가 이어지면 추정치가 사실처럼 굳는다. 코드·`TODO_LIST.md`·`docs/`에 결론을 쓰기 직전에
확인분과 미확인분을 다시 분리하고, 미확인 항목은 "확인 필요"로 남긴다.
