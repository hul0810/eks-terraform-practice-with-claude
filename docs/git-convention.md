...
동일한 root module 디렉토리에서 두 명 이상이 동시에 작업하면 DynamoDB state lock 충돌이 발생한다. 같은 `{env}/{region}/{project}/{resource}` 경로를 편집 중인 사람이 있는지 확인 후 작업한다.
...
provider 버전 범프, 대규모 리팩토링은 기능 변경과 함께 커밋하지 않고 전용 PR로 분리한다.

---

## `.terraform.lock.hcl` 정책

현재 `.gitignore`에 포함되어 있다 (개인 학습 프로젝트 기준).

HashiCorp 공식 권장 사항은 lock 파일을 Git으로 추적하는 것이다. 팀 프로젝트 또는 CI/CD 환경으로 전환 시 `.gitignore`에서 제거하고 추적을 시작한다.
