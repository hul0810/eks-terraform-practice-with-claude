...
| `aws-secrets-store-csi-driver-provider` | Bootstrap | SSM Parameter Store/Secrets Manager Pod 마운트 불가 |
...
| Secrets Store CSI Driver + ASCP | Bootstrap (`aws_eks_addon`) | EKS 커뮤니티 애드온. SSM/Secrets Manager 마운트 전제 조건 |
...
Bootstrap 애드온 7종은 모두 `module "eks"` 내 `addons` 블록에 선언하며, `before_compute` 파라미터로 배포 순서를 제어한다. 별도 서브모듈 호출이나 외부 `aws_eks_addon` 리소스가 불필요하다:
...
| Secrets Store CSI Driver + ASCP | `aws-secrets-store-csi-driver-provider` | 없음 | DaemonSet. 노드 없이는 ACTIVE 불가. IAM은 앱 Pod SA에 별도 부여 |
