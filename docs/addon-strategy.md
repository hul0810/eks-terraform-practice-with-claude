...
| `aws-secrets-store-csi-driver-provider` | EKS 관리형 (조건부) | SSM Parameter Store/Secrets Manager Pod 마운트 불가 |
...
| Secrets Store CSI Driver + ASCP | 없음 | EKS 관리형 (`aws_eks_addon`), IAM은 앱 Pod SA에 별도 부여 |
